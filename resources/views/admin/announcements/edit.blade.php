                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('admin.announcements.update', $announcement->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $announcement->title) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="type" id="type" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    <option value="umum" {{ old('type', $announcement->type) == 'umum' ? 'selected' : '' }}>Umum (Informasi Biasa)</option>
                                    <option value="kegiatan" {{ old('type', $announcement->type) == 'kegiatan' ? 'selected' : '' }}>Kegiatan (Agenda Desa)</option>
                                    <option value="darurat" {{ old('type', $announcement->type) == 'darurat' ? 'selected' : '' }}>Darurat (Penting & Mendesak)</option>
                                </select>
                            ...
                            <div>
                                ...
                                <input type="date" name="expired_at" id="expired_at" value="{{ old('expired_at', $announcement->expired_at ? $announcement->expired_at->format('Y-m-d') : '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        ...
                        <div>
                            ...
                            @if($announcement->attachment)
                                <div class="mb-3 text-sm text-gray-600 bg-gray-50 p-3 rounded-md border border-gray-200">
                                    <strong>File Saat Ini:</strong> 
                                    <a href="{{ Storage::url($announcement->attachment) }}" target="_blank" class="text-indigo-600 hover:underline">Lihat Lampiran</a>
                                    <p class="text-xs text-gray-500 mt-1">Unggah file baru jika ingin mengganti file yang sudah ada.</p>
                                </div>
                            @endif
                        ...
                        <div>
                            ...
                            <div class="mt-1">
                                <textarea id="content" name="content" rows="10" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md">{{ old('content', $announcement->content) }}</textarea>
                            </div>
                        ...
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $announcement->is_active) ? 'checked' : '' }} class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                            </div>
